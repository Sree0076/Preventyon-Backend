...
namespace PreventyonUnitTest
{
    ...
    public class EmployeeEndpointUnitTest
    {
        ...
        public async Task UpdateIncidentByReview_ValidId_ReturnsOk()
        {
            ...
            _incidentRepoMock.Setup(repo => repo.UpdateIncidentAsync(It.IsAny<Incident>())).Returns(Task.FromResult(_incident));
        ...
        [Test]
        public async Task UpdateIncidentByReview_SetToReviewStatus_ReturnsOk()
        {
            // Arrange
            _incidentRepoMock.Setup(repo => repo.GetIncidentById(It.IsAny<int>())).ReturnsAsync(_incident);
            _incidentRepoMock.Setup(repo => repo.UpdateIncidentAsync(It.IsAny<Incident>())).Returns(Task.FromResult(_incident));
            ...
            Assert.IsNotNull(okResult);
            Assert.AreEqual(200, ((Microsoft.AspNetCore.Http.HttpResults.Ok<APIResponse>)okResult).StatusCode);
            Assert.AreEqual("review", _incident.IncidentStatus);
        }

        [Test]
        public async Task IncidentApproval_ValidIncident_ReturnsOk()
        {
            // Arrange
            _incident.IsSubmittedForReview = true;
            _incident.Correction = "Correction made";
            _incidentRepoMock.Setup(repo => repo.GetIncidentById(It.IsAny<int>())).ReturnsAsync(_incident);
            _incidentRepoMock.Setup(repo => repo.UpdateIncidentAsync(It.IsAny<Incident>())).Returns(Task.FromResult(_incident));
            ...
            Assert.IsNotNull(okResult);
            Assert.AreEqual(200, ((Microsoft.AspNetCore.Http.HttpResults.Ok<APIResponse>)okResult).StatusCode);
            Assert.AreEqual("closed", _incident.IncidentStatus);
        }

        [Test]
        public async Task IncidentApproval_NotSubmittedForReview_ReturnsOk()
        {
            // Arrange
            _incident.IsSubmittedForReview = false;
            _incident.Correction = "Correction made";
            _incidentRepoMock.Setup(repo => repo.GetIncidentById(It.IsAny<int>())).ReturnsAsync(_incident);
            _incidentRepoMock.Setup(repo => repo.UpdateIncidentAsync(It.IsAny<Incident>())).Returns(Task.FromResult(_incident));
